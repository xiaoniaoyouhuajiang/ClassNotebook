...
下面使用基于kivy的一个todolist项目的本地运行来讲解相关的内容：
...
##### 打包Python应用
Python应用已经渗透进了不同领域，下面列举一些不同场景下最合适的Python应用打包工具。
* Kivy（客户端打包方案—移动APP/web前端）
* PEX（对于已经安装了Python的环境，要迁移开发环境的最佳方案）
...
一个Python项目一定需要包含的内容是：
* pyproject.toml
* 指定的build system

让我们了解一下通用Python项目的打包方法。
首先，在最外层目录创建`setup.py`以及`setup.cfg`文件，这两个文件的最简化表示如下：
...
从PEP-621开始，python社区统一选择`pyproject.toml`作为项目元信息的标准，setuptools也支持了这一标准，那么问题是两者的依赖关系究竟是怎么样的。简单来说，是setuptools依赖于pyproject.toml，具体来说，跟setuptools相关的主要是[build-system]和[project]两项信息。
...
展示一份标准的基于setuptools的pyproject.toml:
