...
如果derivation定义了`shellHook`变量，nix会在运行了`source $stdenv/setup`之后，运行该定义中的命令，
...
##### 一个C/C++项目的构建
参照nix.dev上提供的[教程](nix.dev/tutorials/packaging-existing-software)，我们来构建一个更加复杂的计算类型的application: uarch-bench
