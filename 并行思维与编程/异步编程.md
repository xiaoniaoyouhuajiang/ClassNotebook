...
generator-rs 库是一个非常典型的、基于栈（Stackful）的协程实现，与本书第五章讨论的纤程（Fiber）/绿色线程属于同一种技术路线。它提供了比书中示例更完整、更健壮、跨平台性也更好的实现。

**generator-rs 库完成的功能（对照书中第五章内容）：**

1. **上下文表示 (RegContext, detail/asm/*.rs, detail/_.rs)**:
   
   - **书中内容：** 定义了 ThreadContext 结构体来保存 callee-saved 寄存器。
...
2. **上下文切换 (detail/asm/*.S, swap_registers)**:
   ...
   - **书中内容：** 使用 Rust 内联汇编 asm! 宏编写 switch 函数，手动 mov 寄存器值并使用 ret 跳转。
...
4. **协程初始化 (initialize_call_frame, gen_init, bootstrap_green_task)**:
   ...
   - **书中内容：** 手动在 spawn 函数中计算偏移量，使用 std::ptr::write 将 guard 和 f 的地址写入栈中，并设置初始 rsp。
...
5. **运行时和调度 (rt.rs, yield_.rs)**:
   ...
   - **书中内容：** 实现了一个简单的 Runtime 结构体和 t_yield 函数进行轮询调度。
