...
namespace stepik {
  ...
  public:
    ...
    vector(std::initializer_list <Type> init) { //для возможности использования в  данном контейнерном классе список инициализации
      if(init.size()) {
    ...
    iterator erase(const_iterator first, const_iterator last) { //удаляет элементы в диапазоне
      ...
      iterator temp = new value_type[lenght];
      std:: copy(m_first, first, temp);
      std:: copy(last, m_last, temp + index);
      delete[] m_first;
      m_first = temp;
      m_last = m_first + lenght;
