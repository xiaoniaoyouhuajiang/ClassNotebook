...
void FlowScene::makeSnapshot() {
    snapshots.push(this->saveToMemory());
    qDebug() << "SS: stack size: " << snapshots.size() << "\n";
}

void FlowScene::back() {
    if (snapshots.isEmpty()) return;

    qDebug() << "back: TOP: "<< snapshots.top() << "\n";
    clearScene();
    loadFromMemory(snapshots.pop());
    qDebug() << "back: stack size: " << snapshots.size() << "\n";
}

void FlowScene::runProject() {
    CacheLog::get_instance().log_data("running graphwindow...");

    try {
        GraphWindow * window = new GraphWindow(this->saveToMemory());
        window->show();
...
void FlowScene::openLogs() {
    QMessageBox msgBox;
       msgBox.setText("Logs");

       QFile json_file("logs.txt");
           json_file.open(QFile::ReadOnly);
           QString logs = json_file.readAll();
           if (logs.length() > 1500)
           logs = logs.right(1500);
           json_file.close();

       //msgBox.setInformativeText(CustomException("Parsing error", "").get_action_error());
       msgBox.setText(logs);
...
std::shared_ptr<Connection> FlowScene::createConnection(Node& nodeIn,
  ...
  auto connection = std::make_shared<Connection>(nodeIn,
                                 portIndexIn,
                                 nodeOut,
                                 portIndexOut,
                                 converter);
