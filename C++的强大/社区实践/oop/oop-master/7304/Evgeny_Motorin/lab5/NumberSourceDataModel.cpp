...
INIT_NUMBER::INIT_NUMBER() : _lineEdit(new QLineEdit()) {
  _lineEdit->setValidator(new QDoubleValidator());

  _lineEdit->setMaximumSize(_lineEdit->sizeHint());

  connect(_lineEdit, &QLineEdit::textChanged,
          this, &INIT_NUMBER::onTextEdited);

  _lineEdit->setText("0.0");
}


QJsonObject INIT_NUMBER::save() const {
  QJsonObject modelJson = NodeDataModel::save();
...
void INIT_NUMBER::restore(QJsonObject const &p) {
  QJsonValue v = p["number"];
