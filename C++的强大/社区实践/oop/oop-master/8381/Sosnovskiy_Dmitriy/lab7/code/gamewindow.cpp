...
void GameWindow::showError(Exception* exc){
    QMessageBox msgBox;
    msgBox.setWindowTitle(exc->getTitle());


    QLabel* text = new QLabel("Error in line: " + QString::number(exc->getLine()));
    QLabel* text1 = new QLabel();

    QLabel* text3 = new QLabel(exc->getDescription());
...
void GameWindow::showWinnerInfo(unsigned int winnerPlayer)
{
    ...
    msgBox.setWindowTitle("Победитель!");

    QLabel* text = new QLabel("Победил игрок номер " + QString::number(winnerPlayer));
...
void GameWindow::on_startGameButton_clicked()
{
    ...
    */
    ...
    QLabel* text = new QLabel("Тип правил игры:");
    QComboBox* rulesTypesBox = new QComboBox();
    rulesTypesBox->addItem("FirstRule");
    rulesTypesBox->addItem("SecondRule");

    StartNewGameData* newGameData;

    msgBox.layout()->addWidget(text);
    msgBox.layout()->addWidget(rulesTypesBox);

    msgBox.exec();

    if(rulesTypesBox->currentIndex() == 0)
        newGameData = new StartNewGameData(FIRST);
    else newGameData = new StartNewGameData(SECOND);

    chainOfResponsibilities->handle(newGameData);

    ui->graphicsView->updateFieldPicture(facade->getRowsAmount(), facade->getColumnsAmount());
