    ...
    , ui(new Ui::MainWindow)
{
    ...
    for(unsigned i = 0; i < game->getField()->getWidth(); i++){
        for(unsigned j = 0; j < game->getField()->getHeight(); j++){
            if (game->getField()->getCell(i,j)->getLandscape()->getLandscapeType()=="river")
                land.append("~   ");
            if (game->getField()->getCell(i,j)->getLandscape()->getLandscapeType()=="mountain")
                land.append("^   ");
            if (game->getField()->getCell(i,j)->getLandscape()->getLandscapeType()=="forest")
                land.append("*   ");
...
void MainWindow::on_actionadd_neutral_triggered()
{
    ...
    addNeutral->ptr = this;
    vector<string> neutrals = {"Visor", "Lat", "Boots", "Ascorbic", "Leonurus", "Belladonna", "Cyanide", "Sword", "Sickle"};
