...
public:
    ...
    FieldPosition createUnit(const std::shared_ptr<const UnitFactory> &unitFactory, FieldPosition basePosition) override {
        ...
        FieldPosition unitPosition = {-1, -1};
        if (base != nullptr)
            unitPosition = base->createUnit(unitFactory, basePosition, *m_field);
        if (unitPosition != FieldPosition{-1, -1})
    ...
    void move(FieldPosition unitPosition, FieldPosition targetPosition) override {
        ...
        if (possibleMoves.find(targetPosition) == possibleMoves.end())
            throw std::runtime_error("Game::move: Impossible move.");
    ...
    void attack(FieldPosition attackerPosition, FieldPosition victimPosition) override {
        auto possibleAttacks = findPossibleAttacks(attackerPosition);
        if (possibleAttacks.find(victimPosition) == possibleAttacks.end())
            throw std::runtime_error("Game::attack: Impossible attack.");
