...
bool Unit::doAttack(Unit*& target) {
	...
	if (this->player == target->getPlayer()) {
		log << (std::string)"Нельзя атаковать своих!\n";
		return false;
	}
	...
	if (abs(this->x - target->x) > localRange || abs(this->y - target->y) > localRange) {
		...
		log << "Нельзя атаковать дальше чем на " + std::to_string(localRange) + " клеток!\n";
		return false;
	}
	...
	log << "Сила атаки: " + std::to_string(std::max(attackPower, 0)) + "\n";
	attacked = true;
...
void Unit::doMove(int x, int y) {
	...
	if (parentField->getTile(x, y).isOccupied || parentField->getTile(x, y).isOccupiedByBase) {
		...
		log << (std::string)"Клетка уже занята!\n";
		return;
	}
	...
	if (this->x >= 0 && (abs(this->x - x) > haste || abs(this->y - y) > haste)) {
		...
		log << "Нельзя передвинуться больше чем на " + std::to_string(haste) + " клеток!\n";
		return;
	}
	if (this->x >= 0) {
...
bool Unit::doAttackOnBase(Base*& target)
{
	...
	log << "Сила атаки: " + std::to_string(std::max(attackPower, 0)) + "\n";
	log << "Прочность базы: " + std::to_string(target->getHp()) + "\n";
