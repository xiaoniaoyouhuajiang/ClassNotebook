...
struct Tile {
	...
	Terrain* terrain;
	bool isOccupied = false;//unit
	Unit* occupiedBy = nullptr;//unit

	bool isOccupiedByBase = false;
	Base* occupiedByBase = nullptr;

	bool isOccupiedByNeutral = false;
	NeutralObject* occupiedByNeutral = nullptr;
	...
	Tile(int x, int y, Terrain*& terrain, bool isOccupiedByNeutral, NeutralObject* occupiedByNeutral, int capt) :x(x), y(y), terrain(terrain), isOccupied(false), occupiedBy(nullptr), isOccupiedByBase(false), occupiedByBase(nullptr), isOccupiedByNeutral(isOccupiedByNeutral), occupiedByNeutral(occupiedByNeutral), capturedBy(capt) {}
};
