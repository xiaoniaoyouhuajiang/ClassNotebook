...
bool ProxyBoardfield::add_landscape(int x, int y, int key) {
    board->log->write(std::string("Adding landscape at (") + std::to_string(x) + "; " + std::to_string(y) + ")");
    return board->add_landscape(x, y, key);
}

bool ProxyBoardfield::add_neutral_object(int x, int y, int key) {
    board->log->write(std::string("Adding neutral object at (") + std::to_string(x) + "; " + std::to_string(y) + ")");
    return board->add_neutral_object(x, y, key);
}
