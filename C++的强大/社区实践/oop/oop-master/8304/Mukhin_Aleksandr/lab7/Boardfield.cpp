...
bool Boardfield::move_unit(int old_x, int old_y, int dest_x, int dest_y) {
    if (!is_valid_coordinates(old_x, old_y)) {
        throw(InteractionError("wrong start coordinates", old_x, old_y));
    }
    if (!is_valid_coordinates(dest_x, dest_y)) {
        throw(InteractionError("wrong end coordinates", dest_x, dest_y));
    }
    if (is_base(dest_x, dest_y) && !is_unit(dest_x, dest_y)) {
        throw(InteractionError("unit can't attack base", old_x, old_y, dest_x, dest_y));
    }
    if (!(is_unit(old_x, old_y) || is_base(old_x, old_y))) {
        throw(InteractionError("only unit or base can attack someone", old_x, old_y));
    }
    if (is_landscape(dest_x, dest_y) && !repository[dest_x][dest_y].landscape->can_stand) {
        throw(InteractionError("there is landscape, where you can't stand", dest_x, dest_y));
    }

    if (!is_free_coordinates(dest_x, dest_y)) {
        if (is_base(old_x, old_y) && !is_unit(old_x, old_y) && is_unit(dest_x, dest_y)) {
            return attack(old_x, old_y, dest_x, dest_y, "base action");
        } else if (is_unit(dest_x, dest_y)) {
            return attack(old_x, old_y, dest_x, dest_y, "unit action");
        }
    }
