...
class ConcreteMediator {
 private:
    ...
    Object** selectedUnit;
    int* currNumBase;
    TestsMediator* test = new TestsMediator();

 public:
    ConcreteMediator(Game<Rules>* game, int h, int w, bool* isUnitSelected, Object** selectedUnit, int* currNumBase)
  ...
  /* + */    test->Move( handler->Move((Unit*)(*selectedUnit), w, h));
  ...
  /* + */     test->Attack(handler->Attack((Unit*)(*selectedUnit), game->field->cells(w,h).innerObject));
              *isUnitSelected = false;
              *currNumBase = !(*currNumBase);
  ...
  /* + */
              test->Move(handler->MoveToNeutr((Unit*)(*selectedUnit), (NeutralObject*)game->field->cells(w,h).innerObject));
              *isUnitSelected = false;
              *currNumBase = !(*currNumBase);
