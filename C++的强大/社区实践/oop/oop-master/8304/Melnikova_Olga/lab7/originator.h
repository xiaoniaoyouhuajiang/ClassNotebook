...
private:
 ...
 public:
  ...
  void Restore(Game<Rules>** game, bool& isUnitSelected, bool& logToFile, bool& logToTerminal, bool& withoutLog, Object** selectedUnit, int& currNumBase)
  {
      ...
      if(fin>>_game.base1->health>>size){
      }else{
          return;
      }
    ...
    if(fin>>unit->health
      ...
      if(!(fin>>_game.base2->health)) return;
      ...
      if(!(fin>>_game.isActionDone>> _game.isAttackDone
              >>isUnitSelected
              >>logToFile
              >>logToTerminal
              >>withoutLog)){
