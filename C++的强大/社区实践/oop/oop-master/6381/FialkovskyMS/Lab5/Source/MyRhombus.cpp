...
void MyRhombus::ChangeScale(float kf) {
    std::cout << "MaxL before of rhombus before: " << maxLenght << std::endl;
    MyShape::ChangeScale(kf);
    std::cout << "Scale of MyRhombus has been changed.\nAfter: " << maxLenght << " " << getObjectID();
}
...
double MyRhombus::getArea() {
    return maxLenght * maxLenght * 1.5 * 0.5;
}