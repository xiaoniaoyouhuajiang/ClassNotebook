...
GameBoard::GameBoard(std::istream& is, InObjectsTable& table) {
	...
	if (info.needWrite) {
		...
		table.getObject(info, [this](void* obj) {
			this->controller = *((GameController*)obj);
		});
	}
	...
	int interceptorsCount;
	Serializer::readInt(is, &interceptorsCount);
	for (int i = 0; i < interceptorsCount; i++) {
		info = table.readInfo(is);
		assert(!info.needWrite);
		table.getObject(info, [this](void*obj) {
			this->interceptors.add(shared_ptr<Interceptor>((Interceptor*)obj));
		});
	}
...
void GameBoard::writeObject(std::ostream& os, OutObjectsTable& table) {
	...
	int interceptorsCount = interceptors.getSize();
	Serializer::writeInt(os, &interceptorsCount);
	for (shared_ptr<Interceptor> ptr : interceptors) {
		Interceptor* interceptor = ptr.get();
		info = table.writeInfo(os, interceptor);
		assert(!info.needWrite);
	}
