...
MouseTracker::MouseTracker(istream& is, InObjectsTable& table) {
	ObjectInfo info;

	info = table.readInfo(is);
	assert(!info.needWrite);
	table.getObject(info, [this](void* obj) {
		this->listener = (MouseTrackerListener*)obj;
	});
...
void MouseTracker::writeObject(ostream& os, OutObjectsTable& table) {
	ObjectInfo info;
