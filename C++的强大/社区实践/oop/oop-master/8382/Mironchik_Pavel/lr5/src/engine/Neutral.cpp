...
Neutral::Neutral(istream& is, InObjectsTable& table) : GameObject(is, table) {
	_texture = Textures::readTexture(is, table);
	_shapeDrawer = ShapeDrawer(is, table);
	Serializer::readInt(is, &_effectDistance);
}

void Neutral::writeObject(ostream& os, OutObjectsTable& table) {
	GameObject::writeObject(os, table);
	Textures::writeTexture(os, table, _texture);
	_shapeDrawer.writeObject(os, table);
	Serializer::writeInt(os, &_effectDistance);
}
...
int Neutral::effectDistance() const {
...
void Neutral::validateShapeDrawer() const {
