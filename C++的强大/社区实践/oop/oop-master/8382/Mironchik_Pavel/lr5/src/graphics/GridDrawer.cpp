...
void GridDrawer::readObject(istream& is, InObjectsTable& table) {
	ObjectInfo info;
	...
	Serializer::read(is, &board, sizeof(sf::Rect<float>));
	Serializer::read(is, &cellsCount, sizeof(sf::Vector2i));
	Serializer::readFloat(is, &gridWidth);
	Serializer::read(is, &cellSize, sizeof(sf::Vector2f));
}

void GridDrawer::writeObject(ostream& os, OutObjectsTable& table) {
	ObjectInfo info;

	info = table.writeInfo(os, viewport);
	assert(!info.needWrite);

	Serializer::write(os, &board, sizeof(sf::Rect<float>));
	Serializer::write(os, &cellsCount, sizeof(sf::Vector2i));
	Serializer::writeFloat(os, &gridWidth);
	Serializer::write(os, &cellSize, sizeof(sf::Vector2f));
}
