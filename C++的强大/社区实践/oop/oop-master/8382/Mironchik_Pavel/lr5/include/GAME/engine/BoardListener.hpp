...
protected:
	void writeBoardListener(ostream& os, OutObjectsTable& table) {
		Serializer::writeInt(os, &_attachState);
	}

	void readBoardListener(istream& is, InObjectsTable& table) {
		Serializer::readInt(is, &_attachState);
	}
