...
Home::Home(istream& is, InObjectsTable& table) : Unit(is, table){
	int count;
	Serializer::readInt(is, &count);
	for (int i = 0; i < count; i++) {
		int childId;
		Serializer::readInt(is, &childId);
		_children.insert(childId);
	}
	Serializer::readInt(is, &_selectedUnit);
...
void Home::writeObject(ostream& os, OutObjectsTable& table) {
	Unit::writeObject(os, table);
	int count = _children.size();
	Serializer::writeInt(os, &count);
	for (int child : _children)
		Serializer::writeInt(os, &child);

	Serializer::writeInt(os, &_selectedUnit);
}
...
bool Home::onClick(sf::Vector2i& cell) {
	if (GameController::instance()->currentState().squad() != getSquad()) {
		setFocused(false);
		return false;
	}
...
void Home::validateSelectedUnitInstance() {
	if (_selectedUnitInstance != nullptr && _selectedUnit == _selectedUnitInstance->info()
