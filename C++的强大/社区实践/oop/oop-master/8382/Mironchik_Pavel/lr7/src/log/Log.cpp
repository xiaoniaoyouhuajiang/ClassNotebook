...
void Log::log(const LogInfo& info) {
	if (!instance()._enabled)
		return;
...
void Log::exception(const LogInfo& info) {
	throw info;
}

void Log::exception(string name, string description) {
	LogInfo info;
	info.name = name;
	info.pushParam("description", description);
	throw info;
}