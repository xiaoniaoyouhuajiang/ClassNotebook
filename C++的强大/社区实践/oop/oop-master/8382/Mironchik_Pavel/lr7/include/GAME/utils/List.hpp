...
void List<T>::insert(T elem, int position) {
	if (!(position <= size && position >= 0)) {
		LogInfo info;
		info.name = "List";
		info.pushParam("description", "Failes insert item: invalid position");
		info.pushParam("position", std::to_string(position));
		info.pushParam("type", typeid(T).name());
		Log::exception(info);
	}
...
T& List<T>::removeAt(int position) {
	if (!(position <= size && position >= 0)) {
		LogInfo info;
		info.name = "List";
		info.pushParam("description", "Failes remove item: invalid position");
		info.pushParam("position", std::to_string(position));
		info.pushParam("type", typeid(T).name());
		Log::exception(info);
	}
