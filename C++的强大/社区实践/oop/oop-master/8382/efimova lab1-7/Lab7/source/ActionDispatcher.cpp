...
#include "Logger.h" //записываем в лог то, что произошло
...
std::vector<ActionDispatcher::Action>* ActionDispatcher::flush() {
	...
	for (int i = 0; i < actionQueue.size(); i++) {
		...
		if (!meta.isEmpty) {
			...
			else if (meta.isCreate) { // обраб. действие - создать юнит
				try { // кладем наш вызов доски в try- catch
					mBoard->addUnit(meta.createdUnit, meta.unitX, meta.unitY);//  вызов к классу доска( созд. юнит и на какие координаты)
				}
				catch (GameException e) {
					Logger::log("ActionDispatcher", e.what());// станд. метод искл., выводит сообщение об ошибке
				}
