...
//У самой доски есть метод getSnapshoе, который собирает все данные о тех юнитах, котор. в ней лежат
QJsonObject Board::getSnapshot() {
	...
	boardProperties.insert("sizeX", mBoardProperties->sizeX);//insert - вставка новых эл.вектора в любое место контейнера
	boardProperties.insert("sizeY", mBoardProperties->sizeY);//вставляем в опред. индекс строки подстроку
	boardState.insert("boardProperties", boardProperties);

	std::vector<std::pair<int, int>> playableUnits;

	//самое веселье
	//сначала мы собираем данные о рельефе,
	//на первом проходе смотрим: если это игровой юнит(не база)- запихиваем в отд.массив
	//если база, то в фин.json
	//если рельеф, то запихиваем в json,который будем сохранять
	...
	//второй проход идет по массиву, в который мы записывали все юниты
	//спрашиваем юнит, к какой базе он привязан
	//идем в фин.json на ячейку базы
	//в базу вкладываем инф., котор. мы скинули
	for (int i = 0; i < playableUnits.size(); i++) {
		...
		//имитация того, как работает json файл
		//сначала идем в одну сторону, изменяем и идем обратно
		auto row = boardArray[baseY].toArray();
		auto cell = row[baseX].toObject();//присвоим тип переменной исходя из иниц. значения
