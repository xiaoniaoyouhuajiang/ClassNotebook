...
RAYGUIDEF bool GuiTextBox(Rectangle bounds, char *text, int textSize, bool editMode)
{
    ...
    Rectangle cursor = {
        ...
        1,
        GuiGetStyle(DEFAULT, TEXT_SIZE)*2.0f
    };
    ...
    if (state == GUI_STATE_PRESSED)
    {
        ...
        if (editMode && ((framesCounter/20)%2 == 0))
        {
            ...
            Rectangle cursor = {
                ...
                1, (float)(GuiGetStyle(DEFAULT, TEXT_SIZE) + GuiGetStyle(TEXTBOX, TEXT_INNER_PADDING))
            };
            ...
            while (!feof(rgsFile))
            {
                switch (buffer[0])
                {
                    case 'p':
                    {
                        ...
                        sscanf(buffer, "p %d %d 0x%d", &controlId, &propertyId, &propertyValue);
